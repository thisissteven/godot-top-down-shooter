...
@export var wall_lights: WallLightPlacer
@export var vertical_window_light_scene: PackedScene
...
func generate() -> void:
    ...
    for child in wall_lights.get_children():
        if child.name.begins_with("WindowLight"):
            child.free()
    ...
    for _w in range(num_windows):
        ...
        while attempts > 0:
            ...
            if horizontal:
                ...
                var first = win_cells.front()
                var last = win_cells.back()
                var center_pos = (
                    map_to_local(first) +
                    map_to_local(last)
                ) * 0.5

                _paint_vertical_window_light(center_pos, win_size)
...
func _paint_vertical_window_light(center_pos: Vector2, win_size: int):
    var vertical_window_light: WindowLight = vertical_window_light_scene.instantiate()
    wall_lights.add_child(vertical_window_light)
    vertical_window_light.strip_length = 12 + 13 + (win_size - 2) * 16
    
    vertical_window_light.name = "WindowLight_V_%d_%d" % [center_pos.x, center_pos.y]
    vertical_window_light.owner = wall_lights.owner
    vertical_window_light.position = center_pos + Vector2(0, 0.5)
